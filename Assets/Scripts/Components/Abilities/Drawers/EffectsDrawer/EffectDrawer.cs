...
namespace Momentum
{
    ...
    public class AbilityEffectDrawer : PropertyDrawer
    {
            ...
            if (EditorGUI.DropdownButton(typeRect, new GUIContent(displayName), FocusType.Keyboard))
            {
                ...
                else
                {
                    foreach (var kvp in typeMap.OrderBy(k => k.Key))
                    {
                        ...
                        {
                            Undo.RecordObject(property.serializedObject.targetObject, "Change Ability Effect");
        ...
        {
            var attr = (EffectCategoryAttribute)System.Attribute.GetCustomAttribute(type, typeof(EffectCategoryAttribute));
