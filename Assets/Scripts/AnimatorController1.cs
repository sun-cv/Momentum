...
// public class AnimatorController1 : Service, IServiceTick, IServiceLoop, IDisposable
// {
...
//     public AnimatorController1(Actor actor)
//     {
...
//         Log.Debug($"animation play X: {((Hero)owner).LockedAim.X} mouse: {((Hero)owner).Intent.Input.Aim.X}");
//         Log.Debug($"animation play Y: {((Hero)owner).LockedAim.Y} mouse: {((Hero)owner).Intent.Input.Aim.Y}");
